...
public class RandomObjectSpanner : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        obstaclesArray = new GameObject[2] {enemy, health};
    ...
    void Update()
    {
        if(currentObstacles.Count >= 10 || PlayerHealthDataStore.getInstance().isPlayerDead())
        {
            return;
        }
        ...
        if(currentObstacles.Count < 2)
        {
            ...
            {
                GameObject defaultObstacle = Instantiate(obstaclesArray[Random.Range(0, 2)]);
        ...
        GameObject newObstacle = Instantiate(obstaclesArray[Random.Range(0, 2)]);
    ...
    //public Vector3 GetRandomPositionOnPlane(GameObject planeGameObject, float borderOffsetPercentage = 2f)
    //{
    //    MeshRenderer planeRenderer = planeGameObject.GetComponent<MeshRenderer>();
    //    Vector3 planeSize = planeRenderer.bounds.size;
